...
class ElectiveClient(BaseClient):
    ...
    def sso_login_dual_degree(self, sida, sttp, referer, **kwargs):
        if len(sida) != 32 or sttp not in ("bzx", "bfx"):
            raise ValueError("Invalid dual-degree login parameters")
        headers = kwargs.pop("headers", {}) # no Referer
        r = self._get(
            url=ElectiveURL.SSOLogin,
            params={
    ...
    def get_PlanController(self, **kwargs):
        ...
        headers = _get_headers_with_referer(kwargs)
        return self._get(
            url=ElectiveURL.ElectivePlanController,
            headers=headers,
            hooks=_hooks_check_title,
            **kwargs,
        )
    ...
    def get_WorkController(self, **kwargs):
        """进入预选阶段选课工作页并建立页面上下文。"""
        headers = _get_headers_with_referer(kwargs, ElectiveURL.ElectivePlanController)
        return self._get(
            url=ElectiveURL.ElectiveWorkController,
            headers=headers,
            hooks=_hooks_check_title,
            **kwargs,
        )

    def get_election(self, page=1, **kwargs):
        """预选阶段可选课程列表。"""
        if page <= 0:
            raise ValueError("page must be positive")
        headers = _get_headers_with_referer(kwargs, ElectiveURL.ElectiveWorkController)
        headers["Cache-Control"] = "max-age=0"
        return self._get(
            url=ElectiveURL.Election,
            params={
                "netui_pagesize": "electableCourseListGrid;20",
                "netui_row": "electableCourseListGrid;%s" % ((page - 1) * 20),
            },
            headers=headers,
            hooks=_hooks_check_title,
            **kwargs,
    ...
    def get_ElectSupplement(self, href, **kwargs):
        ...
        if "/supplement/electSupplement.do" not in href:
            raise RuntimeError("Refusing unexpected election URL: %r" % href)
    ...
    def drop_course(self, href, **kwargs):
        """ 退选一门课 """
        if "/supplement/" not in href or not href.startswith("/"):
            raise RuntimeError("Refusing unexpected drop-course URL: %r" % href)
        headers = _get_headers_with_referer(kwargs, ElectiveURL.SupplyCancel)
        r = self._get(
            url="%s://%s%s" % (ElectiveURL.Scheme, ElectiveURL.Host, href),
            headers=headers,
