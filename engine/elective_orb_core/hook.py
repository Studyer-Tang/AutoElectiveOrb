...
from .parser import get_errInfo, get_tips, get_title, get_tree_from_response
...
def check_status_code(r, **kwargs):
    # requests dispatches response hooks before following redirects.  A valid
    # Location-bearing redirect is therefore an intermediate success, not an
    # HTTP failure.  The final response is checked by the same hook again.
...
def check_elective_title(r, **kwargs):
    if not hasattr(r, "_tree"):
        raise UnexceptedHTMLFormat(response=r, msg="Response tree is missing")
...
def debug_print_request(r, **kwargs):
    ...
    for k, v in r.request.headers.items():
        value = "[REDACTED]" if k.lower() in ("cookie", "authorization", "referer") else v
        cout.debug("%s: %s" % (k, value))
    cout.debug("> Body:")
    cout.debug("[REDACTED]")
    cout.debug("> Response Headers:")
    for k, v in r.headers.items():
        value = "[REDACTED]" if k.lower() in ("set-cookie", "location") else v
        cout.debug("%s: %s" % (k, value))
    cout.debug("")
...
def _dump_request(r):
    ...
    filename = "%s.%s.json.gz" % (timestamp, basename)
    ...
    payload = {
        ...
        "request_headers": {
            key: ("[REDACTED]" if key.lower() in ("cookie", "authorization", "referer") else value)
            for key, value in r.request.headers.items()
        },
        "response_headers": {
            key: ("[REDACTED]" if key.lower() in ("set-cookie", "location") else value)
